# Azure Deployment
## Azure storage displaying micro app containers. 
![Microfrontend Architecture](./images/azurestorage-mfe-container.png)

## Shell app azure pipeline.
![Microfrontend Architecture](./images/shell-app-pipeline-azure.png)

## Azure storage displaying shell App files (hosted as static website $web). 
![Microfrontend Architecture](./images/shell-App-uploaded.png)

## Azure pipeline variables added for all micro apps using access keys from storage account.
![Microfrontend Architecture](./images/profile-app-variables.png)

## Micro app azure pipeline.
![Microfrontend Architecture](./images/user-profile-pipeline-azure.png)

## micro app deployment success.
![Microfrontend Architecture](./images/deploy-success-pipline-profile.png)

## After azure pipeline run completed successfully, files uploaded to azure storage respective container.
![Microfrontend Architecture](./images/profile-app-azure.png)

